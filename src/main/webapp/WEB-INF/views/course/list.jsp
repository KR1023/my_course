...
<!--             <div class="option">
            ...
            <div class="courseList">
	            <c:choose>
	            	...
	            	<p style="text-align:center;margin-top: 200px;width:100%;">등록된 강좌가 없습니다.<p>
	            	</c:when>
	            	<c:otherwise>
                	...
                	<c:otherwise>
                		...
                		<c:choose>
                			<c:when test="${searchName != null }">
                				<a href="/course?page=0&courseName=${searchName }" id="first_page"><img src="./images/arrow/double_arrow_left.svg"></a>
								<c:if test="${list.first == false }">
									<a href="/course?page=${list.number - 1}&courseName=${searchName }" id="prev_page"><img src="./images/arrow/arrow_back.svg"></a>
								</c:if>
								...
								<c:forEach var="i" begin="${start}" end="${end}" step="1">
									<c:choose>
										<c:when test="${(i ) == list.number}">
											<a href="/course?page=${i }&courseName=${searchName }" class="current_page">${i + 1}</a>
										</c:when>
										<c:otherwise>
											<a href="/course?page=${i }&courseName=${searchName }">${i + 1}</a>
										</c:otherwise>
									</c:choose>
								</c:forEach>
								<c:if test="${ list.last == false}">
									<a href="/course?page=${list.number + 1}&courseName=${searchName }" id="next_page"><img src="./images/arrow/arrow_forward.svg"></a>
								</c:if>
								<a href="/course?page=${lastPage}&courseName=${searchName }" id="last_page"><img src="./images/arrow/double_arrow_right.svg"></a>
                			</c:when>
                			<c:otherwise>
                				<a href="/course?page=0" id="first_page"><img src="./images/arrow/double_arrow_left.svg"></a>
								<c:if test="${list.first == false }">
